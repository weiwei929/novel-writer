...
export class VersionManagementService {
   ...
   * 获取版本列表
   */
  async getVersions(projectId: string, query: VersionListQuery = {}): Promise<VersionListResponse> {
    try {
      // 获取版本目录下的所有文件
    ...
    try {
      ...
      if (request.createBackup) {
        const backupResult = await this.createVersion(projectId, {
          ...
          description:
            request.backupDescription || `恢复到版本 ${version.versionNumber} 前的自动备份`,
  ...
  async getVersionStatistics(projectId: string): Promise<any> {
    try {
      ...
      const typeStats = versions.versions.reduce(
        (acc, version) => {
      ...
      const statusStats = versions.versions.reduce(
        (acc, version) => {
