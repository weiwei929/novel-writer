...
class ImportService {
     ...
     * 批量导入多个文件
     */
    async importMultipleFiles(filePaths, options = {}) {
        const results = [];
        const errors = [];
        const warnings = [];
            ...
            if (data.collection && data.projects) {
                ...
                if (firstProject) {
                    return {
                        ...
                        chapters: firstProject.chapters.map((chapter, index) => ({
                            title: chapter.title,
                            content: chapter.content,
                            order: chapter.order || index
                        }))
     ...
     * 导入到数据库
     */
    async importToDatabase(content, options) {
        try {
            ...
            if (options.mergeStrategy !== 'replace') {
                ...
                const existingProject = existingProjects.find(p => p.title.toLowerCase() === content.title.toLowerCase());
                if (existingProject) {
     ...
     * 清理上传文件
     */
    async cleanupUploads(maxAge = 24 * 60 * 60 * 1000) {
        try {
