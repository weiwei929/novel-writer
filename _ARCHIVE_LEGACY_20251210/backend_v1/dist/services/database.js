...
class DatabaseService {
    ...
    ensureProjectMetadata(p) {
        ...
        for (const f of projectFields) {
            if (!p.metadata[f])
                p.metadata[f] = this.createDefaultMetadataItem();
        }
        ...
        if (!Array.isArray(p.chapterPlanning))
            p.chapterPlanning = [];
    }
    ensureChapterMetadata(c) {
        if (!c.metadata)
            c.metadata = {};
        const chapterFields = [
            'synopsis',
        ...
        for (const f of chapterFields) {
            if (!c.metadata[f])
                c.metadata[f] = this.createDefaultMetadataItem();
        }
    }
    ...
    async getProjects(collectionId) {
        ...
        // 确保元数据存在
        list.forEach(p => this.ensureProjectMetadata(p));
        return list;
    }
    async getProjectById(id) {
        await this.init();
        await this.db.read();
        const p = this.db.data.projects.find(p => p.id === id);
        if (!p)
            return null;
    ...
    async getChapters(projectId) {
        ...
        list.forEach(c => this.ensureChapterMetadata(c));
        return list;
    }
    ...
    async search(query, type) {
        ...
        if (!type || type === 'collections') {
            results.collections = this.db.data.collections.filter(c => c.name.toLowerCase().includes(searchTerm) ||
                (c.description && c.description.toLowerCase().includes(searchTerm)) ||
                c.tags.some(tag => tag.toLowerCase().includes(searchTerm)));
        }
        if (!type || type === 'projects') {
            results.projects = this.db.data.projects.filter(p => p.title.toLowerCase().includes(searchTerm) ||
        ...
        if (!type || type === 'chapters') {
            results.chapters = this.db.data.chapters.filter(c => c.title.toLowerCase().includes(searchTerm) ||
    ...
    // ===== 章节规划（Project级结构化数据） =====
    async updateProjectChapterPlanning(projectId, plans) {
        await this.init();
        await this.db.read();
        ...
        if (!Array.isArray(plans))
            throw new Error('Invalid chapter planning payload');
        project.chapterPlanning = plans;
        project.updatedAt = new Date().toISOString();
        await this.db.write();
        return project.chapterPlanning;
    }
    async updateProjectMetadataField(projectId, field, content) {
        await this.init();
        await this.db.read();
        ...
        this.ensureProjectMetadata(project);
        const now = new Date().toISOString();
        const item = project.metadata[field] || this.createDefaultMetadataItem();
    ...
    async getProjectMetadataVersions(projectId, field) {
        await this.init();
        await this.db.read();
        if (!this.isAllowedField(field)) {
            throw new Error('Unsupported metadata field');
    ...
    async getProjectMetadataField(projectId, field) {
        await this.init();
        await this.db.read();
        if (!this.isAllowedField(field)) {
            throw new Error('Unsupported metadata field');
    ...
    async saveProjectMetadataVersion(projectId, field, content, userNote = '', autoSaved = false) {
        await this.init();
        await this.db.read();
        if (!this.isAllowedField(field)) {
            throw new Error('Unsupported metadata field');
    ...
    async updateChapterMetadataField(chapterId, field, content) {
        await this.init();
        await this.db.read();
        if (!this.isAllowedField(field)) {
            throw new Error('Unsupported metadata field');
        ...
        this.ensureChapterMetadata(chapter);
        const now = new Date().toISOString();
        const item = chapter.metadata[field] || this.createDefaultMetadataItem();
        item.current = content || '';
        item.wordCount = item.current ? item.current.trim().split(/\s+/).length : 0;
        item.lastModified = now;
    ...
    async getChapterMetadataVersions(chapterId, field) {
        await this.init();
        await this.db.read();
        if (!this.isAllowedField(field)) {
            throw new Error('Unsupported metadata field');
    ...
    async getChapterMetadataField(chapterId, field) {
        await this.init();
        await this.db.read();
        if (!this.isAllowedField(field)) {
            throw new Error('Unsupported metadata field');
    ...
    async saveChapterMetadataVersion(chapterId, field, content, userNote = '', autoSaved = false) {
        await this.init();
        await this.db.read();
        if (!this.isAllowedField(field)) {
            throw new Error('Unsupported metadata field');
