...
import { VersionType, VersionStatus } from '../types/index.js';
/**
 * 版本管理服务
 */
export class VersionManagementService {
     ...
     * 计算版本统计信息
     */
    async calculateVersionStats(projectId, snapshot, previousVersionId) {
        const stats = {
     ...
     * 创建新版本
     */
    async createVersion(projectId, request) {
        try {
            // 验证项目存在
            const project = await this.dbService.getProjectById(projectId);
            if (!project) {
     ...
     * 获取版本列表
     */
    async getVersions(projectId, query = {}) {
        try {
            // 获取版本目录下的所有文件
            ...
            for (const file of versionFiles) {
                if (file.endsWith('.json')) {
                    ...
                    if (version && version.projectId === projectId) {
                        // 应用过滤条件
                        if (query.type && version.type !== query.type)
                            continue;
                        if (query.status && version.status !== query.status)
                            continue;
                        if (query.tags && !query.tags.some(tag => version.tags.includes(tag)))
                            continue;
                        if (query.search) {
     ...
     * 更新版本信息
     */
    async updateVersion(versionId, request) {
        try {
            const version = await this.loadVersionData(versionId);
            if (!version) {
                return {
     ...
     * 恢复到指定版本
     */
    async restoreVersion(projectId, versionId, request) {
        try {
            // 获取要恢复的版本
            const version = await this.loadVersionData(versionId);
            if (!version || version.projectId !== projectId) {
     ...
     * 比较两个版本
     */
    async compareVersions(sourceVersionId, targetVersionId) {
        try {
     ...
     * 获取版本统计信息
     */
    async getVersionStatistics(projectId) {
        try {
            const versions = await this.getVersions(projectId);
...
//# sourceMappingURL=versionManagementService.js.map