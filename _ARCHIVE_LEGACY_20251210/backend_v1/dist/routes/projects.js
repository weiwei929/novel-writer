...
import { ApiErrorCode, createSuccessResponse, createErrorResponse, ErrorCodeToHttpStatus } from '../types/api.js';
const router = express.Router();
// ===== 项目元数据端点 =====
router.put('/:id/metadata/:field', async (req, res) => {
    try {
        ...
        const response = createErrorResponse(ApiErrorCode.DATABASE_ERROR, error instanceof Error ? error.message : 'Failed to update project metadata');
        res.status(ErrorCodeToHttpStatus[ApiErrorCode.DATABASE_ERROR]).json(response);
    }
});
router.get('/:id/metadata/:field', async (req, res) => {
    try {
        const { id, field } = req.params;
        ...
        const response = createErrorResponse(ApiErrorCode.DATABASE_ERROR, error instanceof Error ? error.message : 'Failed to fetch project metadata');
        res.status(ErrorCodeToHttpStatus[ApiErrorCode.DATABASE_ERROR]).json(response);
    }
});
router.get('/:id/metadata/:field/versions', async (req, res) => {
    try {
        const { id, field } = req.params;
        ...
        const response = createErrorResponse(ApiErrorCode.DATABASE_ERROR, error instanceof Error ? error.message : 'Failed to fetch metadata versions');
        res.status(ErrorCodeToHttpStatus[ApiErrorCode.DATABASE_ERROR]).json(response);
    }
});
// ===== 章节规划（Project级结构化数据） =====
router.put('/:id/chapter-planning', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const response = createErrorResponse(ApiErrorCode.DATABASE_ERROR, error instanceof Error ? error.message : 'Failed to update chapter planning');
        res.status(ErrorCodeToHttpStatus[ApiErrorCode.DATABASE_ERROR]).json(response);
    }
});
router.post('/:id/metadata/:field/save-version', async (req, res) => {
    try {
        const { id, field } = req.params;
        ...
        const response = createErrorResponse(ApiErrorCode.DATABASE_ERROR, error instanceof Error ? error.message : 'Failed to save metadata version');
        res.status(ErrorCodeToHttpStatus[ApiErrorCode.DATABASE_ERROR]).json(response);
    }
});
/**
...
router.get('/', async (req, res) => {
    try {
        ...
        const response = createErrorResponse(ApiErrorCode.DATABASE_ERROR, error instanceof Error ? error.message : 'Failed to fetch projects');
        res.status(ErrorCodeToHttpStatus[ApiErrorCode.DATABASE_ERROR]).json(response);
    }
});
/**
...
router.post('/', async (req, res) => {
    try {
        const { title, description, author, genre = [], tags = [], status = 'draft', collectionId } = req.body;
        // 验证必填字段
        if (!title || title.trim() === '') {
            const response = {
                success: false,
...
router.get('/search', async (req, res) => {
    try {
        ...
        const filteredProjects = projects.filter(p => p.title.toLowerCase().includes(q.toLowerCase()) ||
...
router.get('/:id/chapters', async (req, res) => {
    try {
        ...
        const response = createErrorResponse(ApiErrorCode.DATABASE_ERROR, error instanceof Error ? error.message : 'Failed to fetch chapters');
        res.status(ErrorCodeToHttpStatus[ApiErrorCode.DATABASE_ERROR]).json(response);
    }
});
/**
...
router.post('/:id/chapters', async (req, res) => {
    try {
        ...
        if (!chapterData.title) {
            const response = createErrorResponse(ApiErrorCode.VALIDATION_ERROR, 'Chapter title is required');
        ...
        const response = createErrorResponse(ApiErrorCode.DATABASE_ERROR, error instanceof Error ? error.message : 'Failed to create chapter');
        res.status(ErrorCodeToHttpStatus[ApiErrorCode.DATABASE_ERROR]).json(response);
    }
});
