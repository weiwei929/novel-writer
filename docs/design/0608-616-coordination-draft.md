...
| **典型按钮** | 保存、自动保存 | 提交企划课、接收、完成企划、提交创作室、暂存 |
...
| **企划课 · 企划中内容劳动** | 生长 **作品设定**（616-B 三必一选）+ **作品章节**（616-C）。默认先设定后章节，允许往返 | **内容动作** | 企划中详情 / 设定页 / 章节规划。**仅保存内容**，不改 `planned` / 不放行创作室 |
...
目标态（0608 P0）：详情页 / Tab / 编辑器内不承载「完成企划」「提交创作室」等主管层流程按钮；创意组「提交企划课」是否暂留详情页见 §11。
...
| 内容≠流程 | 「保存创意作品」≠「提交企划课」；提交流程按钮位置见 §4.2 |
...
3. 创意组详情页是否**继续保留**「提交企划课」按钮？（原则位：工作台列表；若暂留须符合 §4.2 过渡四条；目标态是否收敛至列表）
...
| draft-1 | 2026-06-16 | 统一 §4.2 流程按钮位置与详情页过渡规则 |
