# TASK: Commit 0608-phase-closure.md to VPS

> **范围**：仅 docs 操作 — 零代码，零夹带
> **背景**：0608 阶段封版记录已定稿（§5 含 HQ 要求的 WT 原则）。首次提交尝试（f0e5487）只更新了 README.md，遗漏了 closure doc。
...
### 2. 写入 0608-phase-closure.md

路径：`docs/design/0608-constitutional-guidance/0608-phase-closure.md`
...
### 3. 检查并使 README.md 索引正确

查看 f0e5487 后的 README.md 内容：

```bash
cat docs/design/0608-constitutional-guidance/README.md
```

做两件事：
- **确认** closure doc 已出现在文件索引表中（应为 #12 或合适的末尾位置）
- **确认** 进度表已更新反映 P3/P4 完成（如果还是旧的"编审部▶️/文集库📋"，则需更新）

如果索引缺少 closure doc 条目 → 添加一行到跨部门层表格末尾。
如果进度表仍是旧的 → 更新为：
...
如果 f0e5487 已经更新了进度表 → 不修改，只确保 closure doc 在索引中。

### 4. 选择性 Stage + Commit

```bash
# stage 仅相关文档
git add docs/design/0608-constitutional-guidance/0608-phase-closure.md
# 如果 README.md 有修改则也 stage
git add docs/design/0608-constitutional-guidance/README.md

# 确认 staged-only（应当只有 1-2 个文件）
git diff --cached --stat

# commit
git commit -m "docs: add 0608 phase closure record (§5 WT principle per HQ directive)"
...
--- f0e5487 之后 README.md 状态 ---
<README.md 中文件索引和进度表部分的当前内容>
