...
export default function ScrapNote() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedId) {
        await scrapsApi.update(selectedId, {
          content,
        ...
        const created = await scrapsApi.create({
          content,
          note: note.trim() || undefined,
          tags: tagsWithPt,
        })
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <div>
          ...
          <textarea
            ...
            rows={10}
            className="w-full px-3 py-2 border rounded-lg text-sm resize-y min-h-[160px]"
            ...
            {(['none', 'partial', 'complete'] as ScrapProcessingType[]).map(t => (
              <button
                ...
                className={`rounded-lg border px-2 py-1 ${
                  processingType === t ? 'ring-2 ring-amber-400' : ''
                }`}
              >
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          ...
          <button
            ...
            disabled
            title="创意讨论将在第二批实现"
            className="px-4 py-2 border text-sm rounded-lg text-gray-400 cursor-not-allowed"
          >
            引用到讨论（即将推出）
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4 text-sm">
        <section>
          ...
          {cited.filter(s => getScrapProcessingType(s.tags) === 'complete').length === 0 ? (
            ...
            cited
              .filter(s => getScrapProcessingType(s.tags) === 'complete')
              .map(s => {
                const { title: t } = parseScrapContent(s.content)
                return (
                  <button
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500 mt-1">→ 可进入提案</div>
                  </button>
                )
              })
          )}
        ...
        <section>
          <h4 className="text-xs font-semibold text-emerald-700 mb-2">Type 2 · 部分参考</h4>
          {cited.filter(s => getScrapProcessingType(s.tags) === 'partial').length === 0 ? (
            <p className="text-xs text-gray-400">暂无</p>
          ) : (
            cited
              .filter(s => getScrapProcessingType(s.tags) === 'partial')
              .map(s => {
                const { title: t } = parseScrapContent(s.content)
                return (
                  <button
