...
export default function TimelinePanel({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const nextSortOrder = entries.length > 0 ? Math.max(...entries.map(e => e.sortOrder)) + 1 : 0
  ...
  const handleSubmit = async (data: Omit<TimelineInput, 'projectId'>) => {
    ...
    try {
      if (editing) {
        ...
        await worldApi.timeline.create(scope, data)
